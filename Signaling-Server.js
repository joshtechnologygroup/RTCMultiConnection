...
module.exports = exports = function(app, socketCallback) {
    ...
    function onConnection(socket) {
        ...
        checkAuthToken(token, sessionid, type, environment, socket, function (error, authenticated, socket) {
            if (error || !authenticated) {
                console.log("Authentication failed for user " + token);
                return;
            }
            console.log("User " + token + " has been authenticated for " + environment);
...
function checkAuthToken(token, roomId, type, environment, socket, callback) {
    if(token && token !== 'undefined'&& token !== 'null') {
        helper.makeAuthApiCall(token, roomId, type, environment, socket, callback);
    }
    else {
        callback(new Error("Invalid token"), false, socket);
    }
}
